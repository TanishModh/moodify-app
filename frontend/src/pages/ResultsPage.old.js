...
const ResultsPage = () => {
  ...
  return (
    <Box sx={{ width: '100%', minHeight: '100vh', backgroundColor: isDarkMode ? '#121212' : '#f5f5f5' }}>
      <Container maxWidth="lg" sx={{ py: 4 }}>
        <Paper elevation={3} sx={{ p: 3, backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }}>
          <Box sx={{ mb: 4 }}>
            <Typography variant="h4" gutterBottom sx={{ color: isDarkMode ? '#fff' : '#000' }}>
              Your Recommendations
            </Typography>
            <Typography variant="subtitle1" sx={{ color: isDarkMode ? '#fff' : '#000' }}>
  ...
  const [musicPage, setMusicPage] = useState(0);
  const [totalMusicPages, setTotalMusicPages] = useState(0);
  const [moviePage, setMoviePage] = useState(0);
  ...
  useEffect(() => {
    setMusicPage(0);
    setTotalMusicPages(Math.ceil(recommendationData.music.length / itemsPerPage));
                ...
                sx={{ 
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, gap: 2 }}>
                    <Button
                      ...
                      sx={{ backgroundColor: isDarkMode ? '#333' : '#f0f0f0', color: isDarkMode ? '#fff' : '#000' }}
                    >
                      Previous
                    </Button>
                    <Typography sx={{ alignSelf: 'center', color: isDarkMode ? '#fff' : '#000' }}>
                      Page {musicPage + 1} of {Math.ceil(recommendationData.music.filter(item => item.language === musicLanguage).length / itemsPerPage)}
                    </Typography>
                    <Button
                      variant="contained"
                      disabled={musicPage >= Math.ceil(recommendationData.music.filter(item => item.language === musicLanguage).length / itemsPerPage) - 1}
                      onClick={() => setMusicPage(prev => Math.min(Math.ceil(recommendationData.music.filter(item => item.language === musicLanguage).length / itemsPerPage) - 1, prev + 1))}
                      sx={{ backgroundColor: isDarkMode ? '#333' : '#f0f0f0', color: isDarkMode ? '#fff' : '#000' }}
                    >
                      Next
                    </Button>
